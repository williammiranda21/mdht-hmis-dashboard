...
export default function CohortsView({ isAdmin = false, viewerId = null }:
  ...
  // A 401 means the SESSION is gone (signed out elsewhere, or the idle
  // timeout cleared it while this tab sat open) — bounce to login. It must
  // never be read as "table missing": on 2026-08-20 a stale-open tab got the
  // idle 401 and the page wrongly showed the one-time-setup banner.
  const auth401 = (r: Response): Response => {
    if (r.status === 401) { window.location.assign('/login'); throw new Error('401'); }
    return r;
  };
  ...
  const loadDetail = (id: number) => {
    ...
    fetch(`/api/cohorts?id=${id}`)
      ...
      .catch((e) => {
        if (String((e as Error)?.message) === '401') return;
        setMsg('Could not load this cohort.');
      });
  };
        ...
        <>
          <div className="bnl-kpis" style={{ marginTop: 16 }}>
            ...
            <div className="bnl-kpi" style={{ ['--kc' as never]: 'var(--accent)' }}>
              ...
              <div className="bnl-kpi-note">{detail.agg.housed_pct == null ? '—' : `${detail.agg.housed_pct.toFixed(1)}% of cohort`}</div>
            </div>
                  ...
                  {detail.snapshots.map((s) => {
                    ...
                    return (
                      <circle key={s.captured_on} cx={sx(s.captured_on)} cy={sy(pct)} r={3.5}
                        ...
                        <title>{`snapshot ${s.captured_on}: ${pct.toFixed(1)}% housed (n=${s.counts.n ?? '—'})`}</title>
                      </circle>
                    );
                  })}
            ...
            <div className="panel" style={{ marginTop: 16, padding: '12px 18px' }}>
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', height: 84 }}>
                {detail.snapshots.map((s) => {
                  const pct = s.counts.housed_pct ?? 0;
                  return (
                    <div key={s.captured_on} title={`${s.captured_on}: ${pct.toFixed(1)}% housed (n=${s.counts.n ?? '—'})`}
