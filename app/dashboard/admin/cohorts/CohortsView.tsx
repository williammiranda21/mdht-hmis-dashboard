...
/**
 * Client cohorts â€” create a named group, paste hashed PersonalIDs (every ID
 ...
 * per refresh, pipeline/snapshot_cohorts.py). Membership is static â€” housed
...
/** Layer-1 stall thresholds (days) â€” arithmetic, not judgment: a member is
 *  QUIET after this many days without a note; an open next-step is stale
 *  after this many days. */
const NOTE_STALL_D = 14;
const STEP_STALL_D = 14;

/** '2026-08-05' â†’ 'today' / '3d' / '2mo' â€” freshness for the notes column
...
// â”€â”€ monday-style people avatars â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// until 2026-08-12 â€” it served the deterministic "Since first note" ledger,
...
/** The "nobody yet" slot â€” dashed empty circle, monday-style. */
...
interface Member {
  ...
  as_of: string | null;
  /** live CE worklist leg + days on it (ETL fields â€” same source as the bar) */
  ms_stage: string | null; ms_wait: number | null;
  /** last notes (bnl_notes enrichment, cap 5) â€” same shape as the BNL roster */
  notes2?: { body: string; author: string | null; at: string }[] | null;
  /** CE journey dates (ident/assessed/referred/accepted/movein) â€” movein feeds
   *  the digest's "housed this period" line */
  milestones: Record<string, string | null> | null;
  /** when this member was added to the cohort â€” anchor for per-client progress */
  added_at?: string | null;
}
interface Task {
  id: number; pid: string; body: string;
  /** [{id, name}] account snapshots â€” several assignees allowed */
  assignees: { id: string; name: string | null }[];
  status: 'open' | 'done';
  created_by: string | null; created_at: string;
  done_at: string | null; done_by: string | null;
}
/** AI Layer-2 pilot â€” shapes returned by /api/ai/summary. Proposals are
...
interface AiResult {
  ...
  current: boolean;
  cached: boolean;
  /** false = ai_summaries.sql not run yet â€” summaries won't persist */
  cacheOk?: boolean;
}
/** `bnl_access` is only meaningful for NON-admins â€” can_see_bnl() is
...
interface Detail {
  ...
  members: Member[];
  missing: string[];
  /** null â†’ cohort_tasks.sql not run yet (setup hint) */
  tasks: Task[] | null;
  /** per-member note timestamps from the last 30 days (Layer-1 digest input) */
  noteDates: Record<string, string[]>;
  /** per-member FIRST note timestamp ever â€” the case-start anchor */
  firstNote: Record<string, string>;
  access: { user_id: string; granted_at: string }[] | null;
  staff: Staff[];
  manage: boolean;
  /** members exist but every row was RLS-filtered â†’ viewer lacks BNL access */
  restricted?: boolean;
  ...
  agg: {
    ...
    /** did the housing stick â€” from each member's FIRST placement */
    retention?: {
    ...
    /** system-wide journey legs (meta.ce_milestones.housed) â€” the ghost figures */
...
export default function CohortsView({ isAdmin = false, viewerId = null }:
  ...
  // Member click â†’ the SAME client card as the BNL (ClientDrawer). The member
  ...
  // Member status filter (active/housed/inactive) â€” header select, user request
  ...
  // draft assignees for the add form â€” several allowed (user request)
  const [taskAssignees, setTaskAssignees] = useState<string[]>([]);
  // monday-style people picker (fixed popover; fixed so .scroll-pin can't clip
  // it). mode 'draft' = picking for the add form Â· 'task' = reassigning an
  // existing task (admin) Â· 'access' = sharing the cohort (instant grant/revoke)
  ...
  // â”€â”€ AI Layer-2 pilot (per-member summary card in the expanded panel) â”€â”€â”€â”€â”€â”€
  ...
  /** generate:false = cache lookup only (free, no API call) â€” used on expand.
  ...
  // reading emits no DOM events â€” 20s closed it mid-sentence (user report
  ...
  useEffect(() => {
    // An in-flight AI generation counts as activity too â€” the 20s idle
    // collapse must never swallow the panel under a "Generatingâ€¦" spinner
  ...
  // Click-away close (user request): any mousedown OUTSIDE the tasks UI â€”
  // the chip cell, the expanded panel, or the people menu, all marked with
  // data-tasks-ui â€” collapses the panel. mousedown (not click) so it fires
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent) => {
      ...
      // menu) is never swallowed by click-away â€” same guard as the idle timer.
  ...
  // In-place refresh after task/note/access mutations â€” patches only what
  ...
  // "Loadingâ€¦" just because a checkbox-sized thing changed (user report).
  const refreshLite = async () => {
    ...
    } catch { /* keep the current view â€” next full load reconciles */ }
  };
  ...
  return (
    <>
            ...
            <div className="meta">
              {isAdmin
                ? <>Create a group, paste client IDs (every ID in the app is click-to-copy), and track
                    the group&apos;s housing outcomes. Membership is static â€” clients stay after they&apos;re
        ...
        <div style={{ padding: '2px 18px 18px' }}>
          ...
          {isAdmin && (
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 14 }}>
              <input className="finput" placeholder="New cohort nameâ€¦" value={name}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            {cohorts !== null && cohorts.length === 0 && !setupNeeded && (
              <span className="bnl-sub">
                {isAdmin ? 'No cohorts yet â€” create the first one above.' : 'No cohorts have been shared with you yet.'}
              </span>
            )}
          </div>
        ...
        <>
          <div className="bnl-kpis" style={{ marginTop: 16 }}>
            ...
            <div className="bnl-kpi" style={{ ['--kc' as never]: 'var(--accent)' }}>
              ...
              <div className="bnl-kpi-note">{detail.agg.housed_pct == null ? 'â€”' : `${detail.agg.housed_pct.toFixed(1)}% of cohort`}</div>
            </div>
            <div className="bnl-kpi" style={{ ['--kc' as never]: 'var(--warn)' }}>
              ...
              <div className="bnl-kpi-note">{detail.agg.median_days_homeless != null ? `median ${fmtInt(detail.agg.median_days_homeless)}d homeless` : 'â€”'}</div>
            </div>
            <div className="bnl-kpi" style={{ ['--kc' as never]: 'var(--danger)' }}>
              ...
              <div className="bnl-kpi-note">{fmtInt(detail.agg.chronic)} chronic Â· {fmtInt(detail.agg.high_risk)} high risk</div>
            </div>
          </div>

          {/* Cohort CE journey â€” the same proportional bar as the BNL system
          ...
          <div className="panel" style={{ marginTop: 16, padding: '12px 18px' }}>
            <div className="hc-sub" style={{ margin: '0 0 10px' }}>
              Cohort CE journey â€” median days between milestones
            ...
            {detail.agg.legs['ident_movein']?.median != null && (
              <div className="bnl-sub" style={{ marginTop: 6 }}>
                End to end {MS_LABELS['ident'] ?? 'Identified'} â†’ {MS_LABELS['movein'] ?? 'Moved in'}:{' '}
                <b style={{ color: 'var(--strong)' }}>{fmtInt(detail.agg.legs['ident_movein'].median!)}d</b> median
                {detail.agg.legs['ident_movein'].mean != null && <> Â· avg {Math.round(detail.agg.legs['ident_movein'].mean!)}d</>}
                {' '}(n={detail.agg.legs['ident_movein'].n})
                {detail.agg.sys_legs?.['ident_movein']?.median != null &&
                  <> Â· system {fmtInt(detail.agg.sys_legs['ident_movein'].median!)}d</>}
                {' '}Â· above each segment: completed legs Â· below: members on that leg right now, days so far
              </div>
            )}
          </div>

          {/* Trend â€” housed % by ACTUAL event dates (placements / HUD returns),
              reconstructed retroactively; refresh snapshots overlay as dots.
              A dot off the line = the data was edited after that capture
              (backdated move-in, corrected exit) â€” that divergence is signal.
              ...
              <div className="panel" style={{ marginTop: 16, padding: '12px 18px' }}>
                <div className="hc-sub" style={{ margin: '0 0 10px' }}>
                  ...
                  <span className="bnl-sub" style={{ marginLeft: 8, textTransform: 'none', letterSpacing: 0 }}>
                    by actual housing / return dates Â· â— = refresh snapshots (as measured)
                  </span>
                </div>
                <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 130, display: 'block' }} role="img"
                  aria-label={`Cohort housed percentage over time, currently ${last.pct.toFixed(1)}%`}>
                  {/* quarter gridlines */}
                  {[0.25, 0.5, 0.75].map((f) => (
                  ...
                  {detail.snapshots.map((s) => {
                    ...
                    return (
                      <circle key={s.captured_on} cx={sx(s.captured_on)} cy={sy(pct)} r={3.5}
                        ...
                        <title>{`snapshot ${s.captured_on}: ${pct.toFixed(1)}% housed (n=${s.counts.n ?? 'â€”'})`}</title>
                      </circle>
                    );
                  })}
            ...
            <div className="panel" style={{ marginTop: 16, padding: '12px 18px' }}>
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', height: 84 }}>
                {detail.snapshots.map((s) => {
                  const pct = s.counts.housed_pct ?? 0;
                  return (
                    <div key={s.captured_on} title={`${s.captured_on}: ${pct.toFixed(1)}% housed (n=${s.counts.n ?? 'â€”'})`}
                      style={{ flex: 1, maxWidth: 44, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: 2 }}>
                      <span style={{ fontSize: 10.5, fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{pct.toFixed(1)}%</span>
          ...
          {/* Returns after first housing â€” HUD M2 framing (first placement,
              returns within windows; a later re-housing does NOT erase the
              return â€” that is exactly how the SPM counts it). Framed as the
              EVENT ("N returned withinâ€¦"), never as "retention", because a
              member can be housed today AND have a counted return â€” a
            ...
            return (
              <div className="panel" style={{ marginTop: 16, padding: '12px 18px' }}>
                <div className="hc-sub" style={{ margin: '0 0 8px' }}>
                  ...
                  <span className="bnl-sub" style={{ marginLeft: 8, textTransform: 'none', letterSpacing: 0 }}>
                    HUD return test, from each member&apos;s first placement Â· re-housing later doesn&apos;t erase a return (SPM M2 convention)
                  </span>
                </div>
                  ...
                  {rt.horizons.map((h) => {
                    ...
                    return (
                      <span key={h.days} title={h.n
                        ...
                        : `no first placement is ${H_LBL[h.days]} old yet â€” nothing to grade`}>
                        ...
                        {h.n
                          ...
                          : <span className="bnl-sub">â€” too soon</span>}
                      </span>
                    );
                  })}
                  <span className="bnl-sub" style={{ marginLeft: 'auto' }}>
                    {rt.returned_n
                      ? <>{rt.returned_n} return{rt.returned_n === 1 ? '' : 's'} overall{rt.median_days_to_return != null && <> Â· median {fmtInt(rt.median_days_to_return)}d after housing</>}</>
                      : 'no returns recorded'}
                    {' '}Â· <b style={{ color: 'var(--accent)' }}>{detail.agg.housed}</b> of {rt.placed_n} ever-housed are housed today
                  </span>
                </div>
              </div>
            );
          ...
          {/* Layer-1 deterministic activity digest â€” every number is arithmetic
            ...
            return (
              <div className="panel" style={{ marginTop: 16, padding: '12px 18px' }}>
                <div className="hc-sub" style={{ margin: '0 0 8px', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  Activity â€” last {digestWin} days
                  ...
                  <span className="bnl-sub" style={{ textTransform: 'none', letterSpacing: 0, fontWeight: 400 }}>
                    computed from note, next-step, and move-in timestamps â€” nothing inferred
                  </span>
                </div>
                <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', fontSize: 13, alignItems: 'baseline' }}>
                  ...
                  {detail.tasks !== null && (
                    <span>
                      ...
                      {' '}Â· <b className="num">{newWin}</b> created
                      {openStale > 0 && <> Â· <b className="num" style={{ color: 'var(--warn)' }}>{openStale}</b> open &gt;{STEP_STALL_D}d</>}
                    </span>
                  )}
                  {housed > 0 && (
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10 }}>
                    {[...byAssignee.entries()].sort((a, b) => b[1].n - a[1].n).map(([id, e]) => (
                      <span key={id} className="bnl-chip" style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}
                        title={`${e.name ?? 'Unknown'} â€” ${e.n} open next-step${e.n === 1 ? '' : 's'}, oldest ${e.oldest}d`}>
                        <Avatar name={e.name} id={id} size={18} />
                        {(e.name ?? '?').split(' ')[0]} Â· {e.n} open{e.oldest >= STEP_STALL_D ? ` Â· ${e.oldest}d` : ''}
                      </span>
                    ))}
                  </div>
                )}
          ...
          <div className="panel" style={{ marginTop: 16 }}>
            ...
            <div className="panel-h">
              <div>
                <h3>{detail.cohort.name} â€” members</h3>
                <div className="meta">
                  {detail.cohort.description || 'No description.'}
                  {detail.missing.length > 0 && ` Â· âš  ${detail.missing.length} member(s) no longer on the roster`}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'flex-end' }}>
                ...
                {isAdmin && (<>
                  ...
                  {detail.access === null ? (
                    ...
                    <>
                      {detail.access.map((a) => {
                        ...
                        return (
                          <span key={a.user_id} className="bnl-chip"
                            ...
                            title={noBnl
                            ? 'This account has no By-Name List access â€” they will see the cohort but not member names. Grant BNL access in the Users console.'
                            : undefined}>
                            ...
                            <span role="button" style={{ cursor: 'pointer', marginLeft: 6, opacity: .7 }}
                              title="Revoke access"
                              onClick={async () => {
                                if (!window.confirm(`Remove ${nm}'s access to â€œ${detail.cohort.name}â€?`)) return;
                      ...
                      <button className="btn" data-tasks-ui=""
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}
                        title="Share this cohort â€” click people in the list to grant or revoke, several in a row"
                        onClick={(e) => {
                  ...
                  <button className="btn" aria-expanded={addOpen}
                    ...
                    + Add clients {addOpen ? 'â–´' : 'â–¾'}
                  </button>
                  <button className="btn" disabled={busy} onClick={async () => {
                    if (!window.confirm(`Delete cohort â€œ${detail.cohort.name}â€? Members are not affected â€” only the grouping is removed.`)) return;
            ...
            {detail.restricted && (
              <div style={{ padding: '0 18px 10px' }} className="bnl-sub">
                You have access to this cohort, but member details require By-Name List access â€”
            ...
            {fStage && (
              <div style={{ padding: '0 18px 10px' }}>
                <button className="btn" onClick={() => setFStage('')}
                  ...
                  â³ Waiting at {MS_LABELS[fStage] ?? fStage} Â· {detail.members.filter((m) => m.ms_stage === fStage).length} member{detail.members.filter((m) => m.ms_stage === fStage).length === 1 ? '' : 's'} âœ•
                </button>
              </div>
            )}
            {isAdmin && addOpen && (
              <div style={{ padding: '0 18px 12px', display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-start' }}>
                <textarea className="finput" rows={2} autoFocus style={{ minWidth: 320, flex: 1 }}
                  placeholder="Paste hashed client IDs (one per line, or comma/space separated)â€¦"
                ...
                <button className="btn" title="Close â€” a pasted draft is kept"
                  onClick={() => setAddOpen(false)}>âœ•</button>
              </div>
            )}
                ...
                <tbody>
                  {(() => {
                    ...
                    <tr onClick={() => openMember(m)} style={{ cursor: 'pointer' }}
                      ...
                      <td className="num">{m.age ?? 'â€”'}</td>
                      <td><span className={`bnl-chip bnl-${m.status}`}>{m.status}</span></td>
                      <td>{m.ptype && <span className="ty">{m.ptype}</span>} {m.project ?? 'â€”'}{!m.enrolled && m.project ? <span className="bnl-sub"> (former)</span> : ''}</td>
                      <td className="num">{m.days_homeless != null ? fmtInt(m.days_homeless) : 'â€”'}</td>
                      <td className="num">{m.ms_wait != null
                        ? <>{fmtInt(m.ms_wait)}d <span className="bnl-sub">Â· {MS_LABELS[m.ms_stage ?? ''] ?? m.ms_stage}</span></>
                        : <span className="bnl-sub">â€”</span>}</td>
                      <td>
                        ...
                        {/* Layer-1 stall flags â€” each is a sentence with a number */}
                        {quietFlag && (
                          <span className="bnl-fp" style={{ background: 'rgba(234,179,8,0.14)', color: 'var(--warn)' }}
                            title={noteDays === null
                              ? 'No notes on file for this member'
                              : `No note in ${noteDays} days â€” the case has gone quiet`}>
                            {noteDays === null ? 'NO NOTES' : `QUIET ${noteDays}d`}
                          </span>
                        )}
                        {oldestOpen > STEP_STALL_D && (
                          <span className="bnl-fp" style={{ background: 'rgba(234,179,8,0.14)', color: 'var(--warn)' }}
                            title={`Oldest open next-step is ${oldestOpen} days old â€” assigned work nobody has closed`}>
                            STEP {oldestOpen}d
                          </span>
                        )}
                      </td>
                      {/* Last note â€” same treatment as the BNL roster column */}
                      <td style={{ maxWidth: 200 }}
                        ...
                        {(m.notes2?.length ?? 0)
                          ...
                          : <span className="bnl-sub">â€”</span>}
                      </td>
                      {/* Next steps â€” option-B chip (user-picked mockup 2026-08-12):
                          ...
                          status. Amber = open work Â· green = all done Â· dashed =
                      ...
                      <td onClick={(e) => e.stopPropagation()} data-tasks-ui="">
                        {detail.tasks === null
                          ? <span className="bnl-sub" title="Run supabase/cohort_tasks.sql to enable">â€”</span>
                          : (() => {
                            // open work â†’ the people on the OPEN items; all done â†’
                            ...
                            const line2 = state === 'open'
                              ? (oldestOpen > 0 ? `open Â· oldest ${oldestOpen}d` : 'open Â· new')
                            ...
                            return (
                              <button className="btn" aria-expanded={expanded}
                                ...
                                title={expanded ? 'Collapse next steps'
                                  : people.length
                                    ? `${state === 'open' ? 'Open items assigned to' : 'Worked by'} ${people.map((p) => p.name).filter(Boolean).join(', ')} â€” click for the checklist`
                                ...
                                <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                                  ...
                                  {state === 'done' && <span style={{ color: col, fontWeight: 700 }}>âœ“</span>}
                                </span>
                      ...
                      <td className="num">
                        {isAdmin && (
                          <button className="btn" style={{ padding: '0 8px', fontSize: 12 }} title="Remove from cohort"
                            ...
                            onClick={async (e) => {
                              e.stopPropagation();
                              if (!window.confirm(`Remove ${m.name ?? m.pid} from â€œ${detail.cohort.name}â€? Notes and next-step history stay on the client record â€” only the cohort membership is removed.`)) return;
                    ...
                    {expanded && detail.tasks !== null && (
                      <tr>
                        <td colSpan={10} style={{ background: 'var(--card-top)', padding: '10px 18px 14px' }}
                          ...
                          {/* AI case summary (Layer 2) â€” REPLACES the old deterministic
                            ...
                            return (
                              <div style={{ margin: '0 0 12px', padding: '9px 12px 10px', borderRadius: 8,
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                                  <b style={{ fontSize: '.76rem', color: 'var(--primary)', letterSpacing: '.03em' }}>âœ¦ AI CASE SUMMARY</b>
                                  {startLabel && (
                                    <span className="bnl-sub" style={{ fontSize: '.72rem' }}>
                                      <b style={{ color: 'var(--strong)' }}>since {startLabel}</b>{days != null ? ` Â· ${fmtInt(days)}d` : ''}
                                    </span>
                                  )}
                                  <span className="bnl-sub" style={{ fontSize: '.72rem' }}>
                                    AI-generated â€” verify against the notes Â· de-identified before sending
                                  </span>
                                  {a?.summary && (
                                    <span className="bnl-sub" style={{ fontSize: '.72rem' }}>
                                      {a.created_at?.slice(0, 10)}{a.current ? '' : ' Â· thread has changed since'}
                                    </span>
                                  )}
                                  <span style={{ marginLeft: 'auto', display: 'inline-flex', gap: 6, alignItems: 'center' }}>
                                    {isAdmin && (
                                      <button className="tbtn" style={{ fontSize: '.72rem' }}
                                        title="Audit view: the exact de-identified payload that leaves the server for the Claude API â€” no API call is made"
                                    ...
                                    <button className="btn" style={{ padding: '1px 9px', fontSize: 12 }}
                                      ...
                                      title="Send the de-identified thread to Claude (~1-2Â¢); the result is cached until the thread changes"
                                      onClick={() => void aiFetch(m.pid, true)}>
                                      {generating ? 'Generatingâ€¦'
                                        : a?.summary ? (a.current ? 'â†» Regenerate' : 'â†» Update â€” thread changed') : 'âœ¦ Generate'}
                                    </button>
                                  </span>
                                </div>
                                ...
                                {a?.cacheOk === false && (
                                  <div className="bnl-sub" style={{ marginTop: 4, fontSize: '.72rem' }}>
                                    âš  cache table missing â€” run <code>supabase/ai_summaries.sql</code> so summaries persist between visits
                                  </div>
                                )}
                                {a?.summary && (
                                ...
                                {(a?.proposals?.length ?? 0) > 0 && (
                                  <div style={{ marginTop: 8 }}>
                                    <div className="bnl-sub" style={{ fontSize: '.72rem', marginBottom: 4 }}>
                                      Suggested next steps â€” nothing is added until you click
                                    </div>
                                    {a!.proposals.map((p, i) => {
                                      const k = `${m.pid}|${i}`;
                                ...
                                {a && !a.summary && !err && !generating && (
                                  <div className="bnl-sub" style={{ marginTop: 4, fontSize: '.78rem' }}>
                                    No summary yet â€” Generate reads the whole thread and writes what has been
                          ...
                          {[...mTasks].sort((a, b) => (a.status === b.status ? 0 : a.status === 'open' ? -1 : 1))
                            ...
                            <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                              {/* Status button (user request: no checkbox, no strikethrough â€”
                                  a green âœ“ button; done text green, pending text amber) */}
                              <button className="btn" disabled={busy}
                                ...
                                title={done
                                  ? `Completed ${t.done_at?.slice(0, 10) ?? ''} by ${t.done_by ?? 'â€”'} â€” click to reopen`
                                  : 'Mark completed'}
                                onClick={async () => {
                              ...
                              <span className="bnl-sub" style={{ flexShrink: 0, display: 'inline-flex', alignItems: 'center', gap: 6,
                                ...
                                title={((t.assignees?.length ?? 0)
                                  ...
                                  : 'Unassigned') + (isAdmin ? ' â€” click to change who is assigned' : '')}
                                onClick={isAdmin ? (e) => {
                          ...
                          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginTop: 10 }}>
                            <input className="finput" style={{ flex: 1, minWidth: 240 }}
                              placeholder="New next stepâ€¦" value={taskText}
                            ...
                            {(() => {
                              ...
                              return (
                                <button className="btn"
                                  title={chosen.length
                                    ? `${chosen.map((s) => s.display_name || s.email).join(', ')} â€” click to change`
                                    : 'Assign to one or more peopleâ€¦'}
                  ...
                  {detail.members.length === 0 && (
                    <tr><td colSpan={10} className="empty">{isAdmin ? 'No members yet â€” paste IDs above.' : 'No members visible.'}</td></tr>
                  )}
      ...
      {sel != null && !detail && !msg && <div className="panel" style={{ marginTop: 16 }}><div className="hc-none">Loadingâ€¦</div></div>}

      {/* Notes hover card â€” fixed so the table's scroll area can't clip it
      ...
      {notePop && (
        <div className="panel" style={{
          ...
          <div style={{ fontWeight: 700, fontSize: '.85rem', marginBottom: 6 }}>
            {notePop.name} <span className="bnl-sub">Â· last {notePop.notes.length} note{notePop.notes.length === 1 ? '' : 's'}</span>
          </div>
      ...
      {/* People picker â€” monday-style assignee menu (fixed popover + backdrop) */}
      {peoplePick && detail && (
        <>
          {/* Backdrop closes the MENU only â€” marked as task UI so the panel's
          ...
          <div className="panel" onMouseMove={touchTasks} data-tasks-ui="" style={{
            ...
            {(() => {
              ...
              const roster = isAccess
                // Admins already see every cohort â€” a grant would be a no-op.
              ...
              return (
                <>
                  <div className="bnl-sub" style={{ padding: '2px 8px 6px' }}>
                    <b>{isAccess ? 'Share cohort withâ€¦' : 'Assignâ€¦'}</b> Â· click to toggle, several allowed
                  </div>
                  {!isAccess && (
                    <div className="dd-opt" style={{ border: 'none' }}
                      onClick={() => { void apply([], '', false); }}>
                      <AvatarEmpty size={26} /><span className="dd-nm">Clear â€” unassigned</span>
                    </div>
                  )}
                  {roster.map((s) => {
                    ...
                    return (
                      <div key={s.id} className={`dd-opt${on ? ' on' : ''}`} style={{ border: 'none' }}
                        ...
                        {on && <span style={{ color: 'var(--primary)', fontWeight: 700, flexShrink: 0 }}>âœ“</span>}
                      </div>
                    );
                  })}
